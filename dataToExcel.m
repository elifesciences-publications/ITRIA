% Extreme oxidation table statistics
...
for i=1:length(tracks_analysis) %loop through the cells
    frameIdx=tracks_analysis(i).Frame==idxF;%get the index of the chosen timepoint. This can be adapted to multiple timepoints in the future. 
    if sum(frameIdx)==0 %the cell wasn't detected in the frame
        continue;       
    elseif sum(frameIdx)==1 %the cell was detected in the sytox frame
        if tracks_analysis(i).roGFP_ratio_overlap(frameIdx)>0
            j=j+1;
...
headText=sprintf('cell intensity data - frame %d, at %s post %s treatment',...
    idxF,vars.Treat_dT_label{1}{idxF},vars.treatLables{1});
headText=cellstr(headText);
dataMat=[headText,{''},{''},{''}; {'int488'},{'mean roGFP ratio'}, ...
    {'max ratio'},{'min ratio'}; cellFate_array', num2cell(meanOx_array)'];
% dataMat=[int488_array',roGFP_ratio_array',max_roGFP_array',min_roGFP_array'];

save('intensity_data.mat','dataMat','dataStruct');%'cellsProps',

xlswrite('intensity_data_mat.xlsx',dataMat);
